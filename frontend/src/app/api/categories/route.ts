export async function GET() {
    ...
    try {
        // Populate product_categories and their image
        const response = await fetch(`${BASE_URL}/api/categories?populate[product_categories][populate][Image][fields]=url,alternativeText`);
        if (!response.ok) {
